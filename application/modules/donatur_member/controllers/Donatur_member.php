...
class Donatur_member extends MY_Controller {
    
    var $parsing_form_input = array('id','nama_donatur_member','jumlah_donasi','tanggal_donasi');
    var $tablename = 'm_donatur';
    var $pk = 'id';
    
    public function __construct(){
		parent ::__construct();
		$this->load->model('m_donatur_member','ma');
	}
	public function index()
	{
        $data['content'] = 'donatur_member/view_donatur_member'; 
		$data['title'] = $this->data['meta_title'];
        //session
        $data['donatur_member'] = $this->ma->get($this->tablename,$this->pk)->result();
        //var_dump($data['donatur_member']);
	...
	public function edit($id = NULL) {
        $data['content'] = 'donatur_member/edit_donatur_member'; 
		$data['title'] = $this->data['meta_title'];       
        if($id == '' || empty($id) || $id == NULL){
            $data['donatur_member'] = $this->ma->get_new($this->parsing_form_input);
        }else{
            $data['donatur_member'] = $this->ma->get($this->tablename,$this->pk,$id)->row();
        }
        //session
        $data['username'] = $this->session->userdata('username');
        $data['user_group'] = strtoupper(level_help($this->session->userdata('user_group')));
	...
	public function delete() {
        ...
        redirect(base_url('donatur_member'));
    }
    ...
    public function save() {       
        $data = $this->ma->array_from_post(array('id','nama_donatur_member','jumlah_donasi','tanggal_donasi'));
