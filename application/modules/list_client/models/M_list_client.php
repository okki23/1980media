...
class M_list_client extends MY_Model {
  ...
  public function opt_pegawai(){
	...
	public function list_list_client(){
		return $this->db->query('select * from m_list_client')->result();
	}
...
public function save_account($data,$id=NULL,$tablename){
	if($id == '' || empty($id) || $id==NULL){
		...
		if($data['password'] == NULL || empty($data['password']) || $data['password'] == ''){
				$this->db->set('list_clientname',$data['list_clientname']);
			 
				$this->db->set('email',$data['email']);
				$this->db->set('no_telp',$data['email']);  
