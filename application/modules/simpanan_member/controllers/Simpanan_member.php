...
class simpanan_member extends MY_Controller {
  ...
  var $tablename = 't_simpanan';
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('m_simpanan_member','m_sup');
    ...
    public function index() {
        ...
        $data['content'] = 'simpanan_member/simpanan_member_view';
    ...
    public function simpanan_detail_view(){
        $data['title'] = $this->data['meta_title'];
        $data['content'] = 'simpanan_member/simpanan_detail_view';
        $data['id'] = $this->uri->segment(3);
        // var_dump($data['id']);
        // // $data['id_anggota'] =  $this->uri->segment(4);
         
        $data['name'] = $this->db->query("select * from m_anggota where id = '".$data['id']."' ")->row();
        // $sql = "select * from m_anggota where id = '".$data['id']."' ";
        // var_dump($sql);
        $data['listing'] = $this->db->query("select * from t_simpanan where  id_anggota = '".$data['id']."' ")->result();
        //var_dump($data['listing']);
        //session
        $data['username'] = $this->session->userdata('username');
        $data['user_group'] = strtoupper(level_help($this->session->userdata('user_group')));
    ...
    public function simpanan_member_detail_view(){
        $data['title'] = $this->data['meta_title'];
        $data['content'] = 'simpanan_member/simpanan_member_detail_view';
        $data['id'] = $this->uri->segment(3);
        
        $data['id_anggota'] =  $this->uri->segment(4);
         
        $data['name'] = $this->db->query("select * from m_anggota where id = '".$data['id_anggota']."' ")->row();
        $data['listing'] = $this->db->query("select * from t_simpanan_member where  id_anggota = '".$data['id_anggota']."' ")->result();
        //var_dump($data['listing']);
        //session
        $data['username'] = $this->session->userdata('username');
        $data['user_group'] = strtoupper(level_help($this->session->userdata('user_group')));
    ...
    public function rekap() {
        $filename = "rekapitulasi_iuran_wajib_anggota_".date('Y-m-d').".xls";
        ...
        header("Content-Transfer-Encoding: binary ");
         ...
         $data = $this->db->query("select a.*,b.nama_asli from t_simpanan_member a
         left join m_anggota b on b.id = a.id_anggota")->result();
         echo '<table border="1" cellpadding="3" cellspacing="0">
         <tr>
         <td> <b>Nama</b> </td>
         <td> <b>Tanggal Bayar</b> </td>
         <td> <b>Jumlah Setoran</b> </td>
         <td> <b>Status</b> </td>
         </tr> 
         ...
         foreach($data as $key=>$val){
            echo '<tr>';
            echo '<td>'.$val->nama_asli.'</td>';
            echo '<td>'.tanggalan($val->tanggal_bayar).'</td>';
            echo '<td> Rp. '.number_format($val->jumlah_bayar,"0").'</td>';
            echo '<td>'.strtoupper($val->status).'</td>';
            echo '</tr>';
         }
         echo '<tr> 
    ...
    public function simpanan_member_detail_getdata(){
        $dataid = $this->input->post('id');
        $dataidanggota = $this->input->post('id_anggota');
        //$hb = $dataid.'-'.$dataidanggota;
        $sql = $this->db->query("select * from  t_simpanan_member where id = '$dataid' AND id_anggota = '$dataidanggota'")->row();
        echo json_encode($sql);
    }

    public function simpanan_member_detail_delete(){
        $id = $this->uri->segment(3);
        $id_anggota = $this->uri->segment(4);
        $qry = $this->db->query("delete from t_simpanan_member where id = '".$id."' AND id_anggota = '".$id_anggota."' ");
        if($qry){
            echo "<script language=javascript>
            alert('Hapus Data Berhasil');
            window.location='" . base_url('simpanan_member/simpanan_member_detail_view/'.$id.'/'.$id_anggota) . "';
                </script>";
        }
    }

    public function simpanan_member_detail_save(){
        ...
        if($datapos['id'] == '' || $datapos['id'] == NULL || empty($datapos['id'])){
           ...
           if($savesimpan){
            echo "<script language=javascript>
             ...
             window.location='" . base_url('simpanan_member/simpanan_member_detail_view/'.$idsimpan.'/'.$datapos['id_anggota']) . "';
                 </script>";
            } 
            ...
            if($saveupdate){
                echo "<script language=javascript>
                 ...
                 window.location='" . base_url('simpanan_member/simpanan_member_detail_view/'.$datapos['id'].'/'.$datapos['id_anggota']) . "';
                     </script>";
            }   
        }
    ...
    public function simpanan_member_detail_save_edit(){
        $datapos = $this->m_sup->array_from_post($this->parsing_form_input);
    ...
    public function store(){
        ...
        $data['parse_view'] = 'simpanan_member/simpanan_member_store';
    ...
    public function save(){
      ...
      if($save){
        echo "<script language=javascript>
         alert('Simpan Data Berhasil');
         window.location='" . base_url('simpanan_member') . "';
             </script>";
      }
    ...
    public function get_bawahan_foreman($idforeman) {
        ...
        $data['idsimpanan_member'] = $this->session->userdata('idsimpanan_member');
    ...
    public function add() {
        ...
        $data['opt_simpanan_member'] = $this->model_user_management->opt_simpanan_member();
    ...
    public function get_val_peg() {
        ...
        $data = $this->db->get('tb_simpanan_member')->row();
        echo json_encode(simpanan_member_detail_getdata);
    }
    ...
    public function pro_add() {
        ...
        $datapos = array('nrp' => $this->input->post('nrp'),
            ...
            'id_simpanan_member' => $this->input->post('id_simpanan_member'),
