 ...
 */
	...
	public function save_data($data,$tablename){
		...
		return  $this->db->query("insert into t_simpanan (id,id_anggota,tanggal_bayar,jumlah_bayar,status,path_upload) values (null,'".$data['id_anggota']."','".$data['tanggal_bayar']."','".$data['jumlah_bayar']."','".$data['status']."','".$data['path_upload']."') ");
	}

	public function save_data_update($data,$tablename){
		return  $this->db->query("update t_simpanan set path_upload = '".$data['path_upload']."', tanggal_bayar = '".$data['tanggal_bayar']."',jumlah_bayar = '".$data['jumlah_bayar']."',status = '".$data['status']."'  where id = '".$data['id']."' and id_anggota = '".$data['id_anggota']."' ");
